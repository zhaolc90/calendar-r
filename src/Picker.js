...
var Picker = createReactClass({
  ...
  getStartValue: function getStartValue(value) {
    var start = value.split('-')[0]
    return start.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '')
  },
  onInputChange: function onInputChange({ target, ..._ }) {
    var startValue = this.getStartValue(target.value)
    var format = "MM/DD/YYYY"
    var startMoment = moment(startValue, format, true);
    // v.format(formatStr)
    this.setState({
      value:[startMoment,startMoment]
    })
  },
  getCalendarElement: function getCalendarElement() {
    ...
    var mask = props.format + ' - ' + props.format
    var realMask = mask.replace(/(Y|M|D)/g, 9)
    return(
      <div style={{ width: '280px' }}>
        <DateRangeInput
          value={value&&value[0]||moment()}
          selectedValue={value && value[0] || moment()}
          format={'MM/DD/YYYY'}
          onSelect={noop}
          onChange={console.log}
        />
        {React.cloneElement(props.calendar, extraProps)}
      </div>
    )
  },
